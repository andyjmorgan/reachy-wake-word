...
# Clone piper-sample-generator (TTS for synthetic training data)
...
# Download TTS model (high quality, 904 speakers)
...
| Finding | Impact |
|---------|--------|
| 100K training steps | +3% accuracy |
| High-quality TTS model | +1% accuracy |
| 150 speakers (not 100 or 200) | Optimal diversity |
| Clean training (no noise) | Better validation scores |
...
- **TFLite conversion fails** — `onnx-tf` version mismatch with newer ONNX. The ONNX models work fine.
- **Cached data reuse** — Delete `trained_models/<name>/` to force regeneration.

#!/usr/bin/env python3
"""Generate Phase 3B experiment configs - high quality model + 4 wake words."""
...
# Fixed values - NOW WITH HIGH QUALITY MODEL AND 4 WAKE WORDS
BASE_CONFIG = {
    "target_phrase": ["reachy", "reeshy", "rishy", "reechy"],  # All 4 variants
    "piper_sample_generator_path": "/home/localuser/source/reachy/openWakeWord/piper-sample-generator",
    "tts_model_path": "/home/localuser/source/reachy/openWakeWord/piper-sample-generator/models/en-us-libritts-high.pt",  # HIGH QUALITY
    "tts_batch_size": 50,
    "steps": 50000,
    "target_accuracy": 0.7,
    "target_recall": 0.5,
...
for speakers, noise, samples, layer_size in itertools.product(SPEAKERS, NOISE, SAMPLES, LAYER_SIZE):
    ...
    name = f"p3b_spk{speakers}_{noise_str}_smp{samples}_lay{layer_size}"
    ...
    with open(config_path, 'w') as f:
        f.write(f"# Phase 3B: HQ model + 4 words | speakers={speakers}, noise={noise}, samples={samples}, layer={layer_size}\n\n")
...
print(f"Wake words: reachy, reeshy, rishy, reechy")

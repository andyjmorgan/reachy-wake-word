...
if __name__ == '__main__':
    ...
    if args.generate_clips is True:
        ...
        if n_current_samples <= 0.95*config["n_samples"]:
            ...
            tts_models = config.get("tts_model_paths", [config["tts_model_path"]])
            if isinstance(tts_models, str):
                tts_models = [tts_models]
            samples_per_model = (config["n_samples"] - n_current_samples) // len(tts_models)
            for tts_model in tts_models:
                logging.info(f"Generating samples with model: {tts_model}")
                generate_samples(
                    text=config["target_phrase"], max_samples=samples_per_model,
                    model=tts_model,
                    ...
                    file_names=[uuid.uuid4().hex + ".wav" for i in range(samples_per_model)]
                )
                torch.cuda.empty_cache()

            # Resample clips to 16kHz if needed
        ...
        if n_current_samples <= 0.95*config["n_samples_val"]:
            ...
            if isinstance(tts_models, str):
                tts_models = [tts_models]
            samples_per_model = (config["n_samples_val"] - n_current_samples) // len(tts_models)
            for tts_model in tts_models:
                generate_samples(text=config["target_phrase"], max_samples=samples_per_model,
                                 model=tts_model,
                                 batch_size=config["tts_batch_size"],
            ...
            # Resample clips to 16kHz if needed
            logging.info("Resampling test clips to 16kHz...")
            import soundfile as sf
            import librosa
            for wav_file in Path(positive_test_output_dir).glob("*.wav"):
                audio, sr = sf.read(str(wav_file))
                if sr != 16000:
                    audio = librosa.resample(audio, orig_sr=sr, target_sr=16000)
                    sf.write(str(wav_file), audio, 16000)
        ...
        if n_current_samples <= 0.95*config["n_samples"]:
            ...
            samples_per_model = (config["n_samples"] - n_current_samples) // len(tts_models)
            for tts_model in tts_models:
                generate_samples(text=adversarial_texts, max_samples=samples_per_model,
            ...
            # Resample clips to 16kHz if needed
            logging.info("Resampling negative training clips to 16kHz...")
            import soundfile as sf
            import librosa
            for wav_file in Path(negative_train_output_dir).glob("*.wav"):
                audio, sr = sf.read(str(wav_file))
                if sr != 16000:
                    audio = librosa.resample(audio, orig_sr=sr, target_sr=16000)
                    sf.write(str(wav_file), audio, 16000)
        ...
        if n_current_samples <= 0.95*config["n_samples_val"]:
            ...
            # Resample clips to 16kHz if needed
            logging.info("Resampling negative test clips to 16kHz...")
            import soundfile as sf
            import librosa
            for wav_file in Path(negative_test_output_dir).glob("*.wav"):
                audio, sr = sf.read(str(wav_file))
                if sr != 16000:
                    audio = librosa.resample(audio, orig_sr=sr, target_sr=16000)
                    sf.write(str(wav_file), audio, 16000)
